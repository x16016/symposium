...
int main(int argc, const char * argv[]) {
    ...
    //画像ファイルの読み込み
    cv::Mat source_img = cv::imread("008.jpg", cv::IMREAD_COLOR);
    ...
    if (flag2 == 1) {
        ...
        for (j=0; j<tate; j++) {
            for (i=0; i<yoko; i++) {
                ...
                //塗りつぶす方
                cv::circle(result2_img, cv::Point(50 + i*10, flag + j*40), 10, cv::Scalar(hakoB[j][i], hakoG[j][i], hakoR[j][i]), -1, CV_AA);
                //輪郭の方
                cv::circle(result2_img, cv::Point(50 + i*10, flag + j*40), 10, cv::Scalar(0, 0, 0), 1, CV_AA);
